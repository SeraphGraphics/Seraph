...
class Grid:
    ...
    def DrawGrid(self, step_px, scale_x, scale_y, view_x_left, view_x_right, view_y_up, view_y_bottom):
        ...
        for x in np.concatenate((np.arange(0, -view_x_left, -self.x_offset), np.arange(0, view_x_right, self.x_offset))):
            points = []
            points.append(Point(x * scale_x, view_y_up * scale_y, 0, self.color, alpha=self.alpha))
            points.append(Point(x * scale_x, -view_y_bottom * scale_y, 0, self.color, alpha=self.alpha))
            line = Line(points)
            line.draw()

        for y in np.concatenate((np.arange(0, -view_y_bottom, -self.y_offset), np.arange(0, view_y_up, self.y_offset))):
            points = []
            points.append(Point(view_x_right * scale_x, y * scale_y, 0, self.color, alpha=self.alpha))
            points.append(Point(-view_x_left * scale_x, y * scale_y, 0, self.color, alpha=self.alpha))
            line = Line(points)
            line.draw()
        glPopMatrix()
...
class Axes:
    ...
    def __init__(self, x, y, z, x_lim, y_lim, width, color_x, color_y):
        ...
        self.size_px = 15
        self.stroke_size = 1
        self.stroke_size_px = self.size_px / 1.25

    def DrawAxes(self, stepx_px, stepy_px, scale_x, scale_y, view_x_left, view_x_right, view_y_up, view_y_bottom):
        points = []
        points.append(Point(0, view_y_up * scale_y, 0, self.color_x))
        points.append(Point(0, -view_y_bottom * scale_y, 0, self.color_x))
        line = Line(points, self.width)
        line.draw()
        points = []
        points.append(Point(view_x_right * scale_x, 0, 0, self.color_y))
        points.append(Point(-view_x_left * scale_x, 0, 0, self.color_y))
        line = Line(points, self.width)
        line.draw()
        ...
        for x in np.concatenate((np.arange(0, view_x_right, self.num_offset_x),
                                 np.arange(0, -view_x_left, -self.num_offset_x))):
            digits = numDot(x)
        ...
        for y in np.concatenate((np.arange(0, view_y_up, self.num_offset_y),
                                 np.arange(0, -view_y_bottom, -self.num_offset_y))):
            digits = numDot(y)
            num = "{:.{digits}f}".format(y, digits=digits).encode('utf-8')
            putText(0 + 5 / stepx_px, y * scale_y + 5 / stepx_px,
                    Fonts.GLUT_STROKE_ROMAN, self.size, num, rgb_picker(155, 255, 209))
            points = []
            points.append(Point(self.stroke_size / 2, y * scale_y, 0, rgb_picker(0, 255, 209)))
            points.append(Point(-self.stroke_size / 2, y * scale_y, 0, rgb_picker(0, 255, 209)))
            line = Line(points, self.width)
            line.draw()
